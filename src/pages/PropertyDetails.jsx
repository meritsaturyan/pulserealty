...
const TEXT = {
  en: {
    ...
    emailPh: 'Your email...',
    phonePh: 'Your phone for callback...',
    send: 'Send',
    needNamePhone: 'Please enter your full name and phone number.',
    thanksContact: (n, e, p) =>
      `Thanks ${n}, we will contact you${e ? ` at ${e}` : ''} or call you at ${p}`,
    amenitiesTitle: 'Amenities',
    actions: {
      call: '📞 Call',
      request: '📲 Request a Callback',
      other: '🔎 View Other Options',
    },
    location: 'Location',
    ...
    modal: {
      ...
      cancel: 'Cancel',
      send: 'Send',
      need: 'Please enter your name and phone.',
  ...
  ru: {
    ...
    convFailed: 'Ошибка конвертации',
    callUs: '📞 Позвонить',
    onlineChat: '💬 Онлайн-чат',
    namePh: 'Ваше полное имя...',
    emailPh: 'Ваш email...',
    phonePh: 'Ваш телефон для звонка...',
    send: 'Отправить',
    needNamePhone: 'Пожалуйста, введите имя и телефон.',
    thanksContact: (n, e, p) =>
      `Спасибо, ${n}, мы свяжемся с вами${e ? ` по ${e}` : ''} или позвоним по номеру ${p}`,
    amenitiesTitle: 'Удобства',
    actions: {
      call: '📞 Позвонить',
      request: '📲 Заказать звонок',
      other: '🔎 Смотреть другие варианты',
    },
    location: 'Локация',
    recommended: 'Рекомендуемые объекты',
    ...
    modal: {
      ...
      cancel: 'Отмена',
      send: 'Отправить',
      need: 'Пожалуйста, укажите имя и телефон.',
      thanks: (n, p) => `Спасибо, ${n}! Мы перезвоним на ${p}.`,
    },
    notFound: 'Объект не найден.',
    chatHello: 'Здравствуйте! Чем можем помочь?',
    area: '',
    amenList: [
      'Кондиционер',
      'Спа и массаж',
  ...
  hy: {
    ...
    convFailed: 'Փոխարկել չհաջողվեց',
    callUs: '📞 Զանգահարեք',
    onlineChat: '💬 Առցանց չատ',
    namePh: 'Ձեր անունը և ազգանունը...',
    emailPh: 'Ձեր էլ. փոստը...',
    phonePh: 'Ձեր հեռախոսահամարը...',
    send: 'Ուղարկել',
    needNamePhone: 'Խնդրում ենք գրել անունը և հեռախոսահամարը։',
    thanksContact: (n, e, p) =>
      `Շնորհակալություն, ${n}, մենք կապ կհաստատենք${e ? ` ${e} հասցեով` : ''} կամ կզանգահարենք ${p} համարով`,
    amenitiesTitle: 'Հարմարություններ',
    actions: {
      call: '📞 Զանգ',
      request: '📲 Պատվիրել զանգ',
      other: '🔎 Տեսնել այլ տարբերակներ',
    },
    location: 'Գտնվելու վայրը',
    recommended: 'Առաջարկվող գույքեր',
    ...
    modal: {
      ...
      cancel: 'Չեղարկել',
      send: 'Ուղարկել',
      need: 'Խնդրում ենք գրել անուն և հեռախոսահամար։',
      thanks: (n, p) => `Շնորհակալություն, ${n}! Մենք կզանգահարենք ${p} համարով։`,
    },
    notFound: 'Գույքը չի գտնվել։',
    chatHello: 'Բարև, ինչպե՞ս կարող ենք օգնել:',
    area: '',
    amenList: [
      'Օդորակիչ',
      'Սպա և մերսում',
...
const ActionButton = styled.button`
  padding: 12px 16px;
  background-color: ${(p) => (p.$green ? '#28a745' : '#f0ae00')};
  color: #fff;
  border: none;
  border-radius: 6px;
  font-weight: bold;
  ...
  justify-content: center;
  &:hover {
    background-color: ${(p) => (p.$green ? '#23913c' : '#e6a700')};
  }
`;
...
const Input = styled.input`
  ...
  width: 100%;
`;
const SendPhoneButton = styled.button`
  margin-top: 8px;
  padding: 10px 16px;
  background: #f0ae00;
  color: #fff;
...
const LBArrow = styled.button`
  ...
  z-index: 2100;
`;
const LBArrowLeft = styled(LBArrow)`
  left: 12px;
`;
const LBArrowRight = styled(LBArrow)`
  right: 12px;
`;
...
function renderDescriptionBlocks(raw) {
  ...
  return blocks.map((block, i) => {
    const lines = block
      ...
      .filter(Boolean);
    const isBullet =
      lines.length > 0 && lines.every((s) => /^[•\-–—]/.test(s));
...
const PropertyDetails = () => {
  ...
  function getCurrentUserMeta() {
    ...
    const fallbackName =
      (document.querySelector('.header-username')?.textContent || '').trim();
  ...
  const property = useMemo(
    () =>
      (Array.isArray(merged) ? merged : []).find(
  ...
  const images = useMemo(() => {
    ...
    const push = (s) => {
      ...
      if (!isHttp && !isRel && !isBlob && !isData) return;
    ...
    return out;
  }, [property?.image, property?.images, album]);
  ...
  const recList = useMemo(() => {
    ...
    const others = src.filter(
      (p) => p && String(p.id) !== String(property.id)
    );
  ...
  useEffect(() => {
    ...
    return () => {
      alive = false;
      window.removeEventListener('pulse:panos-changed', onChange);
    };
  }, [id]);
  ...
  useEffect(() => {
    const onKey = (e) => {
      if (e.key === 'Escape') setShowPanoFS(false);
    };
  ...
  const pages = Math.max(
    1,
    Math.ceil(
      (Array.isArray(recList) ? recList.length : 0) / VISIBLE
    )
  );
  ...
  const isApartment = /(apartment|flat|բնակարան|квартира)/i.test(
    String(property?.type || '')
  );
  const isForSale = /(sale|sell|վաճառ|продажа)/i.test(
    String(property?.status || property?.dealType || '')
  );
  ...
  const displayedPrice =
    !conversionLoading && !conversionError
      ? Number(
          (convertedPrice ?? property?.price ?? 0)
        )
      : 0;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const rateVal = rates[selectedCurrency.toLowerCase()];
        if (!rateVal) throw new Error('no-rate');
        const usd = Number(property.price || 0);
        const raw = usd * Number(rateVal);
        const value =
          selectedCurrency === 'AMD'
            ? Math.round(raw)
            : Number(raw.toFixed(2));
        if (!aborted) {
          setConvertedPrice(String(value));
          setConversionError(null);
        }
      } catch {
        if (!aborted) {
          setConvertedPrice(null);
          setConversionError('fail');
        }
      } finally {
        if (!aborted) setConversionLoading(false);
      }
    })();
    ...
    const off = chat.onMessage?.((m) => {
      ...
      setChatMessages((prev) => {
        ...
        if (
          ...
          last.text === msg.text &&
          Math.abs((msg.ts || 0) - (last.ts || 0)) < 1200
        ) {
          return prev;
        }
        return [...prev, msg];
  ...
  const handlePhoneSend = async () => {
    ...
    const payload = {
      ...
      title: propTitle || '',
      threadId: chat?.getThreadId?.() || '',
      note: `Details form • ${window.location.href} • ${
        propTitle || ''
      }`.trim(),
    };
  ...
  const submitRequest = () => {
    if (!reqName.trim() || !reqPhone.trim())
      return alert(t.modal.need);
    alert(t.modal.thanks(reqName, reqPhone));
    setShowRequest(false);
    setReqName('');
    setReqPhone('');
  };

  if (!hydrated && !property)
    return (
      <Wrapper>
        <p>Բեռնում…</p>
      </Wrapper>
      ...
      <Layout>
        <LeftTop>
                ...
                {images.map((src, i) => (
                  ...
                  >
                    <img
                      src={
                        src ||
                        'https://via.placeholder.com/160x120?text=Photo'
                      }
                      alt={`thumb-${i}`}
                    />
                  </ThumbSmall>
          ...
          <InfoTop>
            <Info>
              <InfoItem>
                <FaBed /> {property.beds} {t.beds}
              </InfoItem>
              <InfoItem>
                <FaBath /> {property.baths} {t.baths}
              </InfoItem>
              <InfoItem>
                <FaRulerCombined /> {areaVal ? areaVal : '—'}
                {t.area}
              </InfoItem>
              <InfoItem>
          ...
          <DescWrap>
            {renderDescriptionBlocks(property.description)}
          </DescWrap>
        </LeftTop>
        ...
        <Right>
          ...
          <CallButton href="tel:+37477447599">
            {t.callUs}
          </CallButton>
          <ChatButton
            onClick={() => {
              if (!CHAT_ENABLED || !chatAvailable) {
                const msg =
                  lang === 'ru'
                    ? 'Чат сейчас недоступен. Попробуйте позже.'
                    : lang === 'en'
                    ? 'Chat is currently unavailable. Please try again later.'
                    : 'Չատը այժմ հասանելի չէ։ Փորձեք ավելի ուշ։';
                alert(msg);
                return;
              }
              setShowChat(true);
          ...
          <div>
            ...
            <Input
              ...
              required
            />
            <SendPhoneButton onClick={handlePhoneSend}>
              {t.send}
            </SendPhoneButton>
          </div>
          ...
          {showMortgage && (
            <CalcBox>
              <CalcTitle>Հիպոտեկայի հաշվիչ</CalcTitle>
              <CalcGrid>
                <CalcField>
                  <span>
                    Գույքի արժեք / Վարկի գումար ({selectedCurrency})
                  </span>
                  <NumInput
                    type="number"
                    min="0"
                ...
                <CalcField>
                  <span>
                    Սկզբնական մուծում ({selectedCurrency})
                  </span>
                  <NumInput
                    type="number"
                    min="0"
                ...
                <CalcField>
                  <span>Տոկոսադրույք (% տարեկան)</span>
                  <NumInput
                    type="number"
                    min="0"
                    step="0.1"
              ...
              <CalcCTA type="button" disabled>
                Ամսական վճարում՝ {money(monthlyPayment)}
              </CalcCTA>
              <SmallNote>
                Սա նախնական հաշվարկ է և կարող է տարբերվել բանկի
                պայմաններից։
              </SmallNote>
            </CalcBox>
          )}
        ...
        <LeftExtras>
          ...
          <Box>
            <ButtonBar>
              <ActionButton as="a" href="tel:+37477447599" $green>
                {t.actions.call}
              </ActionButton>
              <ActionButton
                type="button"
        ...
        <ModalOverlay onClick={() => setShowRequest(false)}>
          <ModalCard onClick={(e) => e.stopPropagation()}>
            ...
            <ModalActions>
              ...
              <ActionButton onClick={submitRequest}>
                {t.modal.send}
              </ActionButton>
            </ModalActions>
          </ModalCard>
